...
class DireccionService:
    ...
    def list_paged(
        ...
        items = (
            q.order_by(Direccion.Calle.asc().nulls_last(), Direccion.Numero.asc().nulls_last(), Direccion.Id.desc())
             .offset((page - 1) * size)
             .limit(size)
             .all()
        )
    ...
    def resolve_exact(self, calle: str, numero: str, comuna_id: int) -> Direccion | None:
        return (
            ...
            .filter(Direccion.Calle == calle, Direccion.Numero == numero, Direccion.ComunaId == comuna_id)
            .first()
        )
