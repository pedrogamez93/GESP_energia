...
class CompraService:
    ...
    # ======================================================================
    # LISTA ENRIQUECIDA (paginada)  -> retorna COMO EL DETALLE
    # ======================================================================
    def list_full(
        self,
        db: Session,
        ...
        """
        Página de compras con el MISMO enriquecimiento del detalle:
        Servicio/Institución, Región/Comuna, Dirección, MedidorIds y PrimerMedidorId.
        """
        db.execute(text("SET TRANSACTION ISOLATION LEVEL READ UNCOMMITTED;"))

        # --- filtros (re-uso de los de list())
        where_parts: list[str] = ["1=1"]
        params: dict = {}
        ...
        # --- página con JOINs (como get_context)
        rows = (
            db.execute(
                text(
                    f"""
                SELECT
                    ...
                    c.FechaCompra, c.Consumo, c.Costo,
                    c.InicioLectura, c.FinLectura, c.Active,
                    c.UnidadMedidaId, c.Observacion, c.FacturaId,
                    c.EstadoValidacionId, c.RevisadoPor, c.ReviewedAt,
                    c.CreatedByDivisionId, c.ObservacionRevision, c.SinMedidor,
                    ...
                    -- Edificio -> Dirección -> Comuna -> Región
                    efi.Id                       AS EDI_Id,
                    dir.Calle                    AS DIR_Calle,
                    dir.Numero                   AS DIR_Numero,
                    dir.DireccionLibre           AS DIR_DireccionLibre,
                    com.Id                       AS COM_Id,
                    com.Nombre                   AS COM_Nombre,
                    com.RegionId                 AS COM_RegionId,
                    r.Id                         AS R_Id,
        ...
        # --- Compras en la página
        compra_ids = [int(r["Id"]) for r in rows]

        # --- Medidores para esas compras (1 solo roundtrip)
        med_rows = (
            db.execute(
                text(
                    """
                SELECT cm.CompraId, cm.MedidorId
                FROM dbo.CompraMedidor cm WITH (NOLOCK)
                WHERE cm.CompraId IN :ids
                ORDER BY cm.CompraId, cm.Id
                """
                ),
                {"ids": tuple(compra_ids)},
            )
            .mappings()
            .all()
        )

        med_map: dict[int, List[int]] = {}
        for mr in med_rows:
            cid = int(mr["CompraId"])
            mid = int(mr["MedidorId"]) if mr["MedidorId"] is not None else None
            if mid is None:
                continue
            med_map.setdefault(cid, []).append(mid)

        # --- Armar salida enriquecida
        items: List[dict] = []
        for r in rows:
            cid = int(r["Id"])
            mids = med_map.get(cid, [])
            first_mid = mids[0] if mids else None

            # Dirección final
            calle = r.get("DIR_Calle")
            numero = r.get("DIR_Numero")
            dlibre = r.get("DIR_DireccionLibre")
            direccion_libre = dlibre or ("{} {}".format(calle, numero).strip() if (calle or numero) else None)
            ...
            item = {
                ...
                "FacturaId": int(r["FacturaId"]) if r["FacturaId"] is not None else None,
                "EstadoValidacionId": r.get("EstadoValidacionId"),
                "RevisadoPor": r.get("RevisadoPor"),
                "ReviewedAt": _fmt_dt(r.get("ReviewedAt")),
                "CreatedByDivisionId": int(r["CreatedByDivisionId"]) if r["CreatedByDivisionId"] is not None else None,
                "ObservacionRevision": r.get("ObservacionRevision"),
                "SinMedidor": bool(r["SinMedidor"]) if r.get("SinMedidor") is not None else False,

                # Enriquecidos (igual que detalle)
                "ServicioId": int(r["D_ServicioId"]) if r["D_ServicioId"] is not None else None,
                "ServicioNombre": r.get("S_Nombre"),
                "InstitucionId": int(r["S_InstitucionId"]) if r["S_InstitucionId"] is not None else None,
                "RegionId": int(r["R_Id"]) if r["R_Id"] is not None else (int(r["COM_RegionId"]) if r.get("COM_RegionId") is not None else None),
                "EdificioId": int(r["EDI_Id"]) if r["EDI_Id"] is not None else None,
                "NombreOpcional": r.get("D_Nombre"),
                "UnidadReportaPMG": bool(r["D_UnidadReportaPMG"]) if r.get("D_UnidadReportaPMG") is not None else None,
                ...
                "Direccion": {
                    "Calle": calle,
                    "Numero": numero,
                    "DireccionLibre": direccion_libre,
                    "ComunaId": int(r["COM_Id"]) if r["COM_Id"] is not None else None,
