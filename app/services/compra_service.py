...
class CompraService:
    ...
    def list_full(
            ...
            # Cabeceras enriquecidas + MedidorIds agregados
            sql = f"""
            WITH base AS (
            SELECT
                c.Id, c.DivisionId, c.EnergeticoId, c.NumeroClienteId,
            ...
            rows = db.execute(text(sql), {**params, "offset": offset, "size": size}).mappings().all()
            if not rows:
                return total, []
            ...
            ids_csv = ",".join(str(i) for i in compra_ids)  # seguro (ids vienen de la BD)
            ...
            items_sql = f"""
                ...
                OPTION (RECOMPILE)
            """
            items_rows = db.execute(text(items_sql)).mappings().all()
            ...
            # Armar salida final por ítem (igual al detalle)
            out: List[dict] = []
            for r in rows:
                ...
                compra_dict = {
                    ...
                    "Consumo": float(r["Consumo"] or 0),
                    "Costo": float(r["Costo"] or 0),
                    "InicioLectura": _fmt_dt(r["FinLectura" if False else "InicioLectura"]),  # mantiene campos originales
                    "FinLectura": _fmt_dt(r["FinLectura"]),
                    "Active": bool(r["Active"]),
                    ...
                    "Items": items_by_compra.get(int(r["Id"]), []),
                    "Direccion": direccion,
                }
