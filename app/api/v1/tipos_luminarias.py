...
router = APIRouter(prefix="/api/v1/tipos-luminarias", tags=["Luminarias"])
svc = CatalogoSimpleService(TipoLuminaria, has_audit=True)
...
@router.post("", response_model=CatalogoDTO, status_code=status.HTTP_201_CREATED, summary="(ADMIN) Crear tipo de luminaria")
def create_item(payload: CatalogoCreate, db: DbDep, _u: Annotated[UserPublic, Depends(require_roles("ADMINISTRADOR"))]):
    return svc.create(db, payload)

@router.put("/{id}", response_model=CatalogoDTO, summary="(ADMIN) Actualizar tipo de luminaria")
...
@router.delete("/{id}", status_code=status.HTTP_204_NO_CONTENT, summary="(ADMIN) Eliminar tipo de luminaria (soft-delete)")
